...
const Table: FC<ITable> = ({
  ...
  const {
  ...
  } = useTable(
    {
      columns,
      data: records,
      initialState: { pageIndex: currPage },
    },
    useSortBy,
    usePagination
  );
  ...
  const getPages = (currentPage: number, totalPages: number) => {
    ...
    if (startPage < 1 && endPage < totalPages && totalPages >= 5) {
      startPage = 1;
      endPage = 5;
    ...
    const keys = Array.from(Array(endPage + 1 - startPage).keys());
  ...
  return (
    <div className="flex flex-col">
      <div className="-my-2 sm:overflow-visible overflow-x-auto">
              ...
              <tbody {...getTableBodyProps()}>
                {isEmpty(page) && !isLoading ? (
                  <tr className="bg-white">
                    <td
                      className="text-center px-6 py-4 text-sm font-medium text-gray-900 lg:max-w-2xl md:max-w-md sm:max-w-xs max-w-0 whitespace-normal break-words"
                    ...
                    return (
                      ...
                      >
                        {row.cells.map((cell) => {
                          return (
                            <td
                              className="px-6 py-4 text-sm font-medium text-gray-900 lg:max-w-2xl md:max-w-md sm:max-w-xs max-w-0 whitespace-normal break-words"
                              {...cell.getCellProps({
                                style: {
